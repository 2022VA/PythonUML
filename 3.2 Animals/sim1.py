...
class World:
    ...
    def liveALittle(self):
        if self.__thingList != [ ]:
            ...
            if (isinstance(randomThing, Bear) and randomThing.getEnergy() <= 0):
                randomThing = None
    ...
    def countThings(self):
        fish = sum(isinstance(x, Fish) for x in self.__thingList)
        bears = sum(isinstance(x, Bear) for x in self.__thingList)
        return [fish, bears]
...
def mainSimulation():
    ...
    for i in range(worldLifeTime):
        myWorld.liveALittle()
        countObjects = myWorld.countThings()
        reportList.append([i, countObjects[0], countObjects[1]])
    
    with open('report_out.csv', 'w', newline='') as f:
        filewrite = csv.writer(f)
        filewrite.writerow(['Time', 'Fish', 'Bear'])
        filewrite.writerows(reportList)
